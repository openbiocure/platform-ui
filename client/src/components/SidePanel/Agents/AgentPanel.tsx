...
export default function AgentPanel() {
  ...
  const onSubmit = useCallback(
    (data: AgentForm) => {
      ...
      // Only validate provider and model for non-external agents
      if (!isExternal && (!provider || !model)) {
        alert(isExternal)
        return showToast({
      ...
      // Validate external URL for external agents
      if (isExternal && !data.externalUrl) {
        return showToast({
          message: localize('com_agents_missing_external_url') || 'External URL is required for external agents',
          status: 'error',
        });
      }
