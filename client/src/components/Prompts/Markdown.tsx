  ...
  const renderContent = (child: React.ReactNode) => {
    ...
    return parts.map((part, index) =>
      index % 2 === 1 ? (
        <b
          key={index}
          className="ml-[0.5] rounded-lg bg-opencure-orange-100 p-[1px] font-medium text-opencure-orange-800 dark:border-opencure-orange-500/50 dark:bg-transparent dark:text-opencure-orange-400/90"
        >
          {`{{${part}}}`}
        </b>
      ) : (
