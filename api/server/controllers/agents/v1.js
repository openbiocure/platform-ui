...
const createAgentHandler = async (req, res) => {
  try {
    ...
    // Encrypt sensitive externalAuth fields if present
    if (agentData.externalAuth) {
      if (agentData.externalAuth.apiKey) {
        agentData.externalAuth.apiKey = encryptV3(agentData.externalAuth.apiKey);
      }
      if (agentData.externalAuth.password) {
        agentData.externalAuth.password = encryptV3(agentData.externalAuth.password);
      }
    }
...
const updateAgentHandler = async (req, res) => {
  try {
    ...
    // Encrypt sensitive externalAuth fields if present
    if (updateData.externalAuth) {
      if (updateData.externalAuth.apiKey) {
        updateData.externalAuth.apiKey = encryptV3(updateData.externalAuth.apiKey);
      }
      if (updateData.externalAuth.password) {
        updateData.externalAuth.password = encryptV3(updateData.externalAuth.password);
      }
    }
